...
import 'package:mobile/src/utility/MediaController/VideoPreview.dart';
...
class _VideoCaptureState extends State<VideoCapture> {
  ...
  Timer _timer;
  static final LIMITED_TIME = 60; //second
  int _start = LIMITED_TIME;
...
//        );
  ...
  void _onStopButtonPressed() {
    ...
    _stopVideoRecording().then((_) async {
      if (mounted) setState(() {});
      final result =
          await NavigatorUtilities.push(context, VideoPreview(File(videoPath)));
      setState(() {
        _start = LIMITED_TIME;
      });
      if (result.contains('ok')) {
        Navigator.pop(context,
            videoPath + 'andVideoThumbnail' + result.replaceFirst('ok', ''));
      }
//      Fluttertoast.showToast(
...
//      );
    ...
    final String filePath = '$videoDirectory/videos-${currentTime}.mp4';
